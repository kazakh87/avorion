...
uniform sampler2D normalOverlayTexture;
in vec3 tangent;
in vec3 bitangent;
...
void main()
{
    vec4 texBase = texture(platingTexture, texCoord * 1.5);
    vec4 texColor = texBase * color;

#ifdef OVERLAY
    vec4 texOverlay = texture(overlayTexture, texCoord * 1.5);
    ...
    vec3 normalTex = texture(normalTexture, texCoord * 1.5).zyx;
    vec4 normalOverlay = texture(normalOverlayTexture, texCoord);
    normalTex = mix(normalTex, normalOverlay.xyz, normalOverlay.a);
    texColor = mix(texColor, color, normalOverlay.a);
    texBase = mix(texBase, vec4(1), normalOverlay.a);
    ...
    highlightColor.rgb = highlightIntensity * (util0.rgb + vec3(0.6)) * (highlightColor.rgb + 0.3);

    // energy highlights
    outFragColor.rgb = mix(outFragColor.rgb, highlightColor.rgb, highlightColor.a);

#ifdef GLOW_TEXTURE
    // glow
    vec4 glow = texture(glowTexture, texCoord * 1.5);
