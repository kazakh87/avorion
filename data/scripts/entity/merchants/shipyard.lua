...
function update(timeStep)
    for i, job in pairs(runningJobs) do
        ...
        if job.executed >= job.duration then
            ...
            if onServer() then
                local player = Player(job.shipOwner)

                if player then
                    createShip(player, job.singleBlock, job.founder, job.insurance, job.captain, job.styleName, job.seed, job.volume, job.scale, job.material, job.shipName)
                end
            end
